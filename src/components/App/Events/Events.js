...
const Events = () => {
  ...
  const handle = async () => {
    try {
      ...
      // const iscombinedData = filteredData.map((response) => {
      //   const { eventname, eventdescription, eventlocation, eventimage, eventtime, eventlink } = response
      //   return {
      //     eventname, eventdescription, eventlocation, eventimage, eventtime, eventlink
      //   }
      ...
      // console.log("filteredData1",new Date())
      console.log("filteredData",new Date(result.response[0].eventtime).toLocaleDateString()
      // result.response.filter((items)=>items.eventtime.toLocaleDateString== new Date()
      // )
      )
      const data1 = result.response.filter((items)=>new Date(items.eventtime).toLocaleDateString()===new Date().toLocaleDateString())
      console.log("data1",data1)
      Setdata(data1);
      setCombinedData(data1)
  ...
  const columns = [

    {
      ...
      cell: (row) => (
        <div style={{
          ...
          borderRadius: "50%",
          overflow: 'hidden',
        }} onClick={() => { row.eventimage != " " && openmodal(Profile_img + row.eventimage) }}>
          <img
            style={{
              width: '100%',
              height: '100%',
            ...
            src={row.eventimage != " " ? Profile_img + row.eventimage : require('../../../assets/img/defalutavtar.jpg')}
            alt="GroupImage"
          /></div>
      ),
    },
...

    {
      ...
      wrap: true,
      cell: (row) => (
        <span onClick={(e)=>{handleClick(row.eventlink,e)}}>
          {row.eventlink}
