...
import { APiURl } from "../../Services/ApiAddress";
 ...
 function FormEditorstyle1  () {
    ...
    const updateAnswer = update?.Answer
    const updateQuestion = update?.Question
    const id = update?._id

  const [isLoading, setIsLoading] = useState(false);
  const [question, setQuestion] = useState(updateQuestion);
  const [answer, setAnswer] = useState(updateAnswer);
  ...
  const token = sessionStorage.getItem('token');
    ...
    const Addprivacy =()=> {
      ...
      var requestOptions = {
        ...
        fetch(APiURl+"updateFAQ", requestOptions)
